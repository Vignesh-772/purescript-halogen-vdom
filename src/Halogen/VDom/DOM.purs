...
import Halogen.VDom.Types (ElemName(..), FnObject, Namespace(..), ShimmerHolder, PartialState, PartialStateImpl(..), VDom(..), runGraft)
...
buildPartialLayout :: ∀ a w. EffectFn3 (VDomSpec a w) (VDomMachine a w) (PartialState a w) (VDomStep a w)
buildPartialLayout = EFn.mkEffectFn3 \(VDomSpec sp) build partial  -> do
  let (PartialStateImpl st fn) =  (unsafeCoerce partial)
      e = fn st
  buildStep <- EFn.runEffectFn1 build e
  let (Step node state _ hFn) = unsafeCoerce buildStep
  let shouldAddPartial =
        case e of
          Elem _ _ _ _ → true
          Keyed _ _ _ _ → true
          _ -> false
  if shouldAddPartial
    then pure $ mkStep $ Step node state (patchPartial (unsafeCoerce partial) sp.fnObject) hFn
    else pure $ buildStep

patchPartialEvaluated :: ∀ a w. EFn.EffectFn3 (PartialState a w) FnObject (KeyedState a w) (VDomStep a w)
patchPartialEvaluated = mkEffectFn3 \newPartial fnObject state -> do
  let (PartialStateImpl newSt fn) = (unsafeCoerce newPartial)
      vdom = fn newSt
  patchedStep <- EFn.runEffectFn2 (patchKeyed fnObject) state vdom
  let (Step node state _ hFn) = unsafeCoerce patchedStep
  pure $ mkStep $ Step node state (patchPartial (unsafeCoerce newPartial) fnObject) hFn

patchPartial :: ∀ a w. (PartialState a w) -> FnObject -> EFn.EffectFn2 (KeyedState a w) (VDom a w) (VDomStep a w)
patchPartial oldPartial fnObject = EFn.mkEffectFn2 \state vdom → do
  let { build, node } = state
      (PartialStateImpl oldSt _) = (unsafeCoerce oldPartial)
  case vdom of
    (PartialLayout x) ->
                let (PartialStateImpl st _) = (unsafeCoerce x)
